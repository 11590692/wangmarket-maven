/*设置底部的网站相关信息*/
document.getElementById('footerCopyRight').innerHTML = 'Copyright（c）2018 版权所有：'+site['companyName']+' All Rights Reserved';
		...
		for (var i = 0; i < siteColumnRank.length; i++) {
			...
			for (var j = 0; j < siteColumn.length; j++) {
				if (siteColumn[j]['id'] == siteColumnRank[i]) {
					...
					case '3':
						/* 独立页面 */
						urledit = '<a href="'+masterSiteUrl+'news/updateNewsByCid.do?cid='+siteColumn[j]['id']+'">' + siteColumn[j]['name'] +"</a>";
						break;
					case '4':
						/* 留言板 */
						break;
...
/*2016.10.9升级*/
try{
	document.getElementById('qrImg').innerHTML='<img id="qrImage" src="'+resBasePath+'image/loading.svg" style="height: 150px;" alt="二维码"><div id="footerQrImage">微&nbsp;信&nbsp;扫&nbsp;一&nbsp;扫</div>';
	document.getElementById('fenxiangdao').innerHTML='分享到：';
}catch(err){}
/*动态加载底部又下角的二维码*/
document.getElementById('qrImage').src=attachmentFileUrl+'images/qr.jpg?v='+time;
